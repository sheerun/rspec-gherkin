...
describe RspecGherkin::StepDefinition do
  let(:all_steps) { [] }

  describe "#match" do
    it "matches a simple step" do
      step = RspecGherkin::StepDefinition.new("there are monsters") {}
    ...
    it "matches placeholders" do
      RspecGherkin::Placeholder.stub(:resolve).with(:count).and_return(/\d+/)
      step = RspecGherkin::StepDefinition.new("there are :count monsters") {}
    ...
    it "extracts arguments from matched steps" do
      step = RspecGherkin::StepDefinition.new("a :age year old monster called :name") {}
    ...
    it "can reuse the same placeholder multiple times" do
      step = RspecGherkin::StepDefinition.new("a the monsters :name and :name") {}
    ...
    it "can reuse the same custom placeholder multiple times" do
      RspecGherkin::Placeholder.stub(:resolve).with(:count).and_return(/\d+/)
      RspecGherkin::Placeholder.stub(:apply).with(:count, "3").and_return(3)
      RspecGherkin::Placeholder.stub(:apply).with(:count, "2").and_return(2)
      step = RspecGherkin::StepDefinition.new(":count monsters and :count knights") {}
    ...
    it "does search for the same custom placeholder several times" do
      placeholder = RspecGherkin::Placeholder.add(:count) { match(/\d/) { |count| count.to_i } }
      RspecGherkin::Placeholder.stub(:resolve).with(:count).and_return(/\d+/)
      RspecGherkin::Placeholder.should_receive(:find).with(:count).twice.and_return(placeholder)
      step = RspecGherkin::StepDefinition.new(":count monsters and :count knights") {}
      match = step.match("3 monsters and 2 knights")
      #match.params.should eq([3, 2])
    end
    ...
    it "matches quoted placeholders" do
      step = RspecGherkin::StepDefinition.new("there is a monster named :name") {}
    ...
    it "matches alternative words" do
      step = RspecGherkin::StepDefinition.new("there is/are monsters") {}
      step.should match("there are monsters")
    ...
    it "matches several alternative words" do
      step = RspecGherkin::StepDefinition.new("monsters are cool/nice/scary") {}
    ...
    it "matches optional parts of words" do
      step = RspecGherkin::StepDefinition.new("there is/are X monster(s)") {}
      step.should match("there is X monster")
      step.should_not match("there is X monsterQ")
    end

    it "matches optional words" do
      step = RspecGherkin::StepDefinition.new("there is a (scary) monster") {}
      ...
      step = RspecGherkin::StepDefinition.new("there is a monster (that is scary)") {}
      step.should match("there is a monster that is scary")
      step.should match("there is a monster")

      step = RspecGherkin::StepDefinition.new("(there is) a monster") {}
      step.should match("there is a monster")
